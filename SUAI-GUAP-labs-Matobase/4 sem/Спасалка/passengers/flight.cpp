...
private:
    ...
    void reverseInorderTraversal(TreeNode* node, std::vector<int>& leftPart, std::vector<int>& rightPart) {
        if (node == nullptr)
            return;

        // Обход левой части: начинайте с левого поддерева
        reverseInorderTraversal(node->left, leftPart, rightPart);
        // Если текущий узел меньше корня, добавляем в левую часть
        if (node->data < root->data) {
            leftPart.push_back(node->data);  // Сохраняем значение узла
        }
        // Если текущий узел больше корня, добавляем в правую часть
        else if (node->data > root->data) {
            rightPart.push_back(node->data);  // Сохраняем значение узла
        }

        // Обход правой части: начинайте с правого поддерева
        reverseInorderTraversal(node->right, leftPart, rightPart);
    }
...
public:
    ...
    std::vector<int> reverseInorderTraversal() {
        ...
        reverseInorderTraversal(root, leftPart, rightPart);

        // Соединяем левую и правую части, добавляем корень в конце
        leftPart.insert(leftPart.end(), rightPart.rbegin(), rightPart.rend());
...
int main() {
    ...
    while (true) {
        ...
        case 5:
            ...
            for (int value : tree.reverseInorderTraversal())
                std::cout << value << " ";
            std::cout << std::endl;
            break;
        case 6:
