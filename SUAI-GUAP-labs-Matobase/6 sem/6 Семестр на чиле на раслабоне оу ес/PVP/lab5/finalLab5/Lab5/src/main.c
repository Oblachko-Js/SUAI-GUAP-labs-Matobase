...
int main(void)
{
	...
	while (1)
	{
		...
		ProcessFsmInterTm1();    //итерация автомата машины тьюринга 1
		ProcessFsmInterTm2();    //итерация автомата машины тьюринга 2
		ProcessFsmInterTm3();    //итерация автомата машины тьюринга 3
	...
	switch (fsmstate)
	{
		case FSMST_WAITKEY:
			if(GetMessage(MSG_KEYPRESSED,(void*)&ParamPtr))
			{
				...
				if (kypd4status & (1<<0))
				{
						SendMessage(MSG_TM1STOP, NULL);
				}
				if (kypd4status & (1<<1))
				{
						SendMessage(MSG_TM2STOP, NULL);
				}
						if (kypd4status & (1<<2))
				{
						SendMessage(MSG_TM3STOP, NULL);
				}
				if(count>0)
				{
					textbuf[count++]='\n';
