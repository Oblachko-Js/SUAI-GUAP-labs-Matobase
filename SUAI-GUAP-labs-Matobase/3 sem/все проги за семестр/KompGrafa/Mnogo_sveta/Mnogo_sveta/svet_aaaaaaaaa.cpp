...
void display() {
    ...
    glRotatef(angleX, 1.0f, 0.0f, 0.0f);
    glRotatef(angleY, 0.0f, 1.0f, 0.0f);
    float roofColor2[] = { 0.0f, 0.0f, 1.0f }; // Синяя крыша
    float wallColor2[] = { 1.0f, 1.0f, 0.0f }; // Желтые стены
    drawHouse(roofColor2, wallColor2, DIFFUSE);
    glPopMatrix();

    // Вращаем и рисуем третий домик
    glPushMatrix();
    glTranslatef(-2.5f, 0.0f, 0.0f); // Изменено с -5.0f на -2.5f
    glEnable(GL_LIGHT2);
    GLfloat lightIntensity3[] = { 0.0f, 0.0f, 1.5f, 1.0f }; // Синий свет
    GLfloat lightPosition3[] = { 0.0f, 2.0f, 2.0f, 1.0f }; // Синий свет
    glLightfv(GL_LIGHT3, GL_DIFFUSE, lightIntensity3);
    ...
    glRotatef(angleX, 1.0f, 0.0f, 0.0f);
    glRotatef(angleY, 0.0f, 1.0f, 0.0f);
    float roofColor3[] = { 1.0f, 1.0f, 1.0f }; // Белая крыша
    float wallColor3[] = { 0.6f, 0.6f, 0.6f }; // Серые стены
