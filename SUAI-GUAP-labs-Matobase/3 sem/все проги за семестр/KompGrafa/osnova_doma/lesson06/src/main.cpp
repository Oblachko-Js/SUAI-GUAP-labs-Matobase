...
bool GLWindowInit(const GLWindow &window)
{
	...
	// кубик
	MeshCreateCube(meshes[1], vec3(0.0f, 1.2f, 0.0f), 1.0f);
	...
	// тор, присоединенный к кубику сверху
	MeshCreateSphere(meshes[2], vec3(4.0f, 2.2f, 0.0f), 0.6f);
...
void GLWindowUpdate(const GLWindow &window, double deltaTime)
{
	...
	// Углы поворота для кубика
static float3 cubeRotation = {0.0f, 0.0f, 0.0f};

// Углы поворота для тора
static float3 torusRotation = {0.0f, 0.0f, 0.0f};
...
// Задаем матрицу вращения кубика
meshes[1].rotation = GLFromEuler(cubeRotation[0], cubeRotation[1], cubeRotation[2]);

// Углы поворота тора с учетом углов поворота кубика
torusRotation[0] = cubeRotation[0];
torusRotation[1] = cubeRotation[1];
torusRotation[2] = cubeRotation[2];

// Задаем матрицу вращения тора
