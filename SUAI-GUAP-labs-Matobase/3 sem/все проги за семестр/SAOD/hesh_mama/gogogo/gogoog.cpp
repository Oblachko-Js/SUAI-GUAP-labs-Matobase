...
bool add(KeyValuePair t[n], const string& k, const string& v) {
    if (!validateKey(k)) {
        cout << "Неверный формат ключа. Используйте формат БцццББ.\n";
        return false;
    }
    ...
    while (i < n) {
        ...
        if (!strcmp(t[new_id].key, "\0") || t[new_id].isDeleted) {
            strcpy(t[new_id].key, key);
            strcpy(t[new_id].value, v.c_str());
...
bool del(KeyValuePair t[n], const string& k) {
    ...
    strcpy(t[index].key, "\0");
    strcpy(t[index].value, "\0");
...
int main() {
    ...
    while (choice != 0) {
        ...
        case 1: {
            for (int i = 0; i < n; i++) {
                strcpy(table[i].key, "\0");
                strcpy(table[i].value, "\0");
        ...
        case 2: {
            ...
            do {
                ...
                if (q == 0) {
                    ...
                    cout << "Ввод ключа (0 для выхода, формат: БцццББ): ";
                    cin >> key;
                    if (key[0] == '0') {
                        break;
                    }
                    ...
                    if (!validateKey(key)) {
                        cout << "Неверный формат ключа. Ключ должен соответствовать формату БцццББ\n";
